...
/*
 * Registers that are identical across all four supported modes.
 * Written first; the per-mode array is written afterwards to
 * override only the registers that differ between resolutions.
 */
static const struct ov5675_reg ov5675_common_regs[] = {
	/* Software reset, standby */
	{0x0103, 0x01}, {0x0100, 0x00},
	/* PLL / clock */
	{0x0300, 0x04}, {0x0302, 0x8d}, {0x0303, 0x00}, {0x030d, 0x26},
	/* Analog control */
	{0x3002, 0x21}, {0x3107, 0x23},
	{0x3501, 0x20}, {0x3503, 0x0c}, {0x3508, 0x03}, {0x3509, 0x00},
	{0x3600, 0x66}, {0x3602, 0x30}, {0x3610, 0xa5}, {0x3612, 0x93},
	{0x3620, 0x80}, {0x3642, 0x0e}, {0x3661, 0x00},
	/* 0x3662 (binning) is mode-specific */
	{0x3664, 0xf3}, {0x3665, 0x9e}, {0x3667, 0xa5},
	{0x366e, 0x55}, {0x366f, 0x55},
	{0x3670, 0x11}, {0x3671, 0x11}, {0x3672, 0x11}, {0x3673, 0x11},
	/* Sensor timing (mode-specific: 0x3714, 0x37c2, 0x37d9) */
	{0x371a, 0x3e}, {0x3733, 0x10}, {0x3734, 0x00}, {0x373d, 0x24},
	{0x3764, 0x20}, {0x3765, 0x20}, {0x3766, 0x12},
	{0x37a1, 0x14}, {0x37a8, 0x1c}, {0x37ab, 0x0f},
	{0x37cb, 0x00}, {0x37cc, 0x00}, {0x37cd, 0x00}, {0x37ce, 0x00},
	{0x37d8, 0x02}, {0x37dc, 0x04},
	/* Crop/window: H_CROP_START, H_CROP_END, V_CROP_END common part */
	{0x3800, 0x00}, {0x3801, 0x00}, {0x3802, 0x00},
	{0x3804, 0x0a}, {0x3805, 0x3f}, {0x3806, 0x07},
	/* HTS=750 (common); VTS is mode-specific and set per-mode table */
	{0x380c, 0x02}, {0x380d, 0xee},
	/* Increment regs common to 2x-binned modes; overridden for 1080p/1x */
	{0x3815, 0x01}, {0x3817, 0x01}, {0x381e, 0x02},
	{0x3821, 0x01},
	{0x3832, 0x04},
	/* Strobe / FREX initial */
	{0x3c80, 0x01}, {0x3c82, 0x00}, {0x3c83, 0xc8},
	{0x3c8c, 0x0f}, {0x3c8d, 0xa0}, {0x3c90, 0x07},
	{0x3c91, 0x00}, {0x3c92, 0x00}, {0x3c93, 0x00},
	{0x3c94, 0xd0}, {0x3c95, 0x50}, {0x3c96, 0x35}, {0x3c97, 0x00},
	/* BLC common */
	{0x4001, 0xe0}, {0x4040, 0x00},
	{0x400f, 0x80}, {0x4013, 0x02}, {0x404c, 0x50}, {0x404e, 0x20},
	/* VFIFO */
	{0x4500, 0x06}, {0x4503, 0x00}, {0x450a, 0x04},
	/* MIPI */
	{0x4809, 0x04}, {0x480c, 0x12}, {0x4819, 0x70},
	{0x4825, 0x32}, {0x4826, 0x32}, {0x482a, 0x06},
	{0x4833, 0x08}, {0x4837, 0x0d},
	/* ISP */
	{0x5000, 0x77},
	/* LSC */
	{0x5b00, 0x01}, {0x5b01, 0x10}, {0x5b02, 0x01}, {0x5b03, 0xdb},
	{0x5b05, 0x6c}, {0x5e10, 0xfc},
	/* AEC/AGC fine-tuning (common values) */
	{0x3500, 0x00}, {0x3503, 0x08}, {0x3508, 0x04}, {0x3509, 0x00},
	{0x3832, 0x48},
	/* DPC */
	...
	{0x5793, 0x52}, {0x5794, 0xa3},
	/* BLC redo, MIPI clock select final */
	{0x4003, 0x40}, {0x3107, 0x01},
	/* Strobe / FREX final */
...
/*
 * Mode-specific registers: only the values that differ from the common table.
 * These are written after ov5675_common_regs to override the relevant fields.
 */

/* 1296x972 — 2x2 binned, full FOV */
static const struct ov5675_reg ov5675_1296x972_mode_regs[] = {
	{0x3662, 0x08},         /* analog binning: 2x2 */
	{0x3714, 0x28},         /* sensor ctrl */
	{0x37c2, 0x14},         /* PCLK period, 2x2 */
	{0x37d9, 0x04},
	/* V_CROP_START */
	{0x3803, 0x00},
	/* V_CROP_END */
	{0x3807, 0xb7},
	/* Output size: 1296x972 */
	{0x3808, 0x05}, {0x3809, 0x10},
	{0x380a, 0x03}, {0x380b, 0xcc},
	/* VTS=2000 */
	{0x380e, 0x07}, {0x380f, 0xd0},
	/* Window offsets */
	{0x3811, 0x08}, {0x3813, 0x0d},
	/* H/V increment: 2x binning */
	{0x3814, 0x03}, {0x3815, 0x01},
	{0x3816, 0x03}, {0x3817, 0x01},
	/* TIMING_FORMAT1: binning on */
	{0x3820, 0x8b},
	/* BLC lines */
	{0x4008, 0x00}, {0x4009, 0x07}, {0x4041, 0x03},
	/* Exposure */
	{0x3501, 0x1f}, {0x3502, 0x20},
};

/* 1920x1080 — cropped, no binning */
static const struct ov5675_reg ov5675_1920x1080_mode_regs[] = {
	{0x3662, 0x10},         /* analog binning: none */
	{0x3714, 0x24},         /* sensor ctrl full-res */
	{0x37c2, 0x04},         /* PCLK period, no binning */
	{0x37d9, 0x08},
	/* V_CROP_START */
	{0x3803, 0x04},
	/* V_CROP_END */
	...
	/* VTS=2020 */
	{0x380e, 0x07}, {0x380f, 0xe4},
	/* Window offsets */
	{0x3811, 0x10}, {0x3813, 0x0d},
	/* H/V increment: no binning */
	{0x3814, 0x01}, {0x3815, 0x01},
	{0x3816, 0x01}, {0x3817, 0x01},
	/* TIMING_FORMAT1: no binning */
	{0x3820, 0x88},
	/* BLC lines */
	{0x4008, 0x02}, {0x4009, 0x0d}, {0x4041, 0x07},
	/* Exposure */
	{0x3501, 0x3e}, {0x3502, 0x60},
};

/* 1280x720 — 2x2 binned, 16:9 crop */
static const struct ov5675_reg ov5675_1280x720_mode_regs[] = {
	{0x3662, 0x08},         /* analog binning: 2x2 */
	{0x3714, 0x24},
	{0x37c2, 0x14},         /* PCLK period, 2x2 */
	{0x37d9, 0x04},
	...
	{0x4008, 0x02}, {0x4009, 0x0d}, {0x4041, 0x07},
	/* Exposure */
	{0x3501, 0x2c}, {0x3502, 0x00},
};

/* 640x480 — 4x4 binned, VGA */
static const struct ov5675_reg ov5675_640x480_mode_regs[] = {
	{0x3662, 0x04},         /* analog binning: 4x4 */
	{0x3714, 0x28},
	{0x37c2, 0x24},         /* PCLK period, 4x4 */
	{0x37d9, 0x02},
	/* V_CROP_START */
	{0x3803, 0x00},
	/* V_CROP_END */
	...
	/* Window offsets */
	{0x3811, 0x08}, {0x3813, 0x0d},
	/* H/V increment: 4x binning */
	{0x3814, 0x07}, {0x3815, 0x01},
	{0x3816, 0x07}, {0x3817, 0x01},
	/* TIMING_FORMAT1: 4x binning on */
	{0x3820, 0x8f},
	/* BLC lines */
	{0x4008, 0x00}, {0x4009, 0x03}, {0x4041, 0x03},
	/* Exposure */
	{0x3501, 0x3e}, {0x3502, 0x60},
};
...
struct ov5675_mode {
	uint16_t width;
	uint16_t height;
	const struct ov5675_reg *mode_regs;
	size_t mode_regs_len;
};

static const struct ov5675_mode ov5675_modes[] = {
	{
		.width        = 1296,
		.height       = 972,
		.mode_regs     = ov5675_1296x972_mode_regs,
		.mode_regs_len = ARRAY_SIZE(ov5675_1296x972_mode_regs),
	},
	{
		.width        = 1920,
		.height       = 1080,
		.mode_regs     = ov5675_1920x1080_mode_regs,
		.mode_regs_len = ARRAY_SIZE(ov5675_1920x1080_mode_regs),
	},
	{
		.width        = 1280,
		.height       = 720,
		.mode_regs     = ov5675_1280x720_mode_regs,
		.mode_regs_len = ARRAY_SIZE(ov5675_1280x720_mode_regs),
	},
	{
		.width        = 640,
		.height       = 480,
		.mode_regs     = ov5675_640x480_mode_regs,
		.mode_regs_len = ARRAY_SIZE(ov5675_640x480_mode_regs),
	},
};
...
static int ov5675_set_fmt(const struct device *dev, enum alif_video_endpoint ep,
			  struct video_format *fmt)
{
	ARG_UNUSED(ep);
...
static int ov5675_get_fmt(const struct device *dev, enum alif_video_endpoint ep,
			  struct video_format *fmt)
{
	ARG_UNUSED(ep);
...
static const struct video_format_cap ov5675_fmts[] = {
	{
		...
		.width_min = 1296, .width_max = 1296,
		.height_min = 972, .height_max = 972,
		.width_step = 0,   .height_step = 0,
	},
	{
		.pixelformat = VIDEO_PIX_FMT_Y10P,
		.width_min = 1920, .width_max = 1920,
		.height_min = 1080, .height_max = 1080,
		.width_step = 0,   .height_step = 0,
	},
	{
		.pixelformat = VIDEO_PIX_FMT_Y10P,
		.width_min = 1280, .width_max = 1280,
		.height_min = 720, .height_max = 720,
		.width_step = 0,   .height_step = 0,
	},
	{
		.pixelformat = VIDEO_PIX_FMT_Y10P,
		.width_min = 640, .width_max = 640,
		.height_min = 480, .height_max = 480,
		.width_step = 0,   .height_step = 0,
	},
	{0},
};

static int ov5675_get_caps(const struct device *dev, enum alif_video_endpoint ep,
...
ALIF_VIDEO_API(ov5675_driver_api,
...
#define ALIF_VIDEO_REGISTER(inst) \
	VIDEO_DEVICE_DEFINE(ov5675_vdev_##inst, DEVICE_DT_INST_GET(inst), NULL);

DT_INST_FOREACH_STATUS_OKAY(ALIF_VIDEO_REGISTER)
