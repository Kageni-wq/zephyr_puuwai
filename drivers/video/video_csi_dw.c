...
static int csi2_dw_stream_start(const struct device *dev)
{
	...
	if (config->sensor[data->current_sensor]) {
		ret = alif_video_stream_start(config->sensor[data->current_sensor]);
		if (ret) {
...
static int csi2_dw_stream_stop(const struct device *dev)
{
	...
	if (config->sensor[data->current_sensor]) {
		ret = alif_video_stream_stop(config->sensor[data->current_sensor]);
		if (ret) {
...
static int csi2_dw_set_format(const struct device *dev, enum alif_video_endpoint ep,
			      struct video_format *fmt)
{
	...
	if (config->sensor[data->current_sensor]) {
		ret = alif_video_set_format(config->sensor[data->current_sensor], ep, fmt);
		if (ret) {
...
static int csi2_dw_get_format(const struct device *dev, enum alif_video_endpoint ep,
		       struct video_format *fmt)
{
	const struct csi2_dw_config *config = dev->config;
	struct csi2_dw_data *data = dev->data;
	...
	if (config->sensor[data->current_sensor]) {
		ret = alif_video_get_format(config->sensor[data->current_sensor], ep, fmt);
		if (ret) {
...
static int csi2_dw_get_caps(const struct device *dev, enum alif_video_endpoint ep,
			    struct video_caps *caps)
{
	 ...
	 */
	return alif_video_get_caps(config->sensor[data->current_sensor], ep, caps);
}
...
static int csi2_dw_set_ctrl(const struct device *dev, unsigned int cid, void *value)
{
	...
	case VIDEO_CID_ALIF_CSI_CURR_CAM:
		{
			...
			video_find_vdev(dev)->src_dev = config->sensor[tmp];
	...
	if (config->sensor[data->current_sensor]) {
		 ...
		 */
		if (ret) {
			ret = alif_video_set_control(config->sensor[data->current_sensor], cid, value);
			if (ret) {
				LOG_ERR("CID Is neither supported by pipeline, nor by CSI IP.");
				return ret;
			}
		} else {
			alif_video_set_control(config->sensor[data->current_sensor], cid, value);
		}
	} else {
		LOG_ERR("Invalid sensor selected!");
		return -ENODEV;
...
static int csi2_dw_get_ctrl(const struct device *dev, unsigned int cid, void *value)
{
	...
	if (!ret) {
		alif_video_get_control(config->sensor[data->current_sensor], cid, value);
		return 0;
	} else {
		return alif_video_get_control(config->sensor[data->current_sensor], cid, value);
	}
}

ALIF_VIDEO_API(csi2_dw_driver_api,
...
#define ALIF_VIDEO_REGISTER(inst) \
	VIDEO_DEVICE_DEFINE(video_csi_dw_vdev_##inst, DEVICE_DT_INST_GET(inst), CSI2_GET_SENSOR(inst, 0));
