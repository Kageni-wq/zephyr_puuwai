...
#define DT_DRV_COMPAT aptina_mt9m114_alif
...
LOG_MODULE_REGISTER(mt9m114_alif, CONFIG_VIDEO_LOG_LEVEL);
...
#if defined(CONFIG_MT9M114_ALIF_PARALLEL_INIT)
...
#if (!IS_ENABLED(CONFIG_MT9M114_ALIF_PARALLEL_INIT))
static struct mt9m114_reg mt9m114_1288_728[] = {
	...
	{/* NULL terminated */}};
#endif /* (!IS_ENABLED(CONFIG_MT9M114_ALIF_PARALLEL_INIT)) */
...
static int mt9m114_set_fmt(const struct device *dev, enum alif_video_endpoint ep,
			   struct video_format *fmt)
{
...
static int mt9m114_get_fmt(const struct device *dev, enum alif_video_endpoint ep,
			   struct video_format *fmt)
{
	struct mt9m114_data *drv_data = dev->data;
...
static int mt9m114_get_caps(const struct device *dev, enum alif_video_endpoint ep,
...
ALIF_VIDEO_API(mt9m114_driver_api,
...
static int mt9m114_init(const struct device *dev)
{
	...
	ret = mt9m114_set_fmt(dev, ALIF_VIDEO_EP_OUT, &fmt);
	if (ret) {
...
#define ALIF_VIDEO_REGISTER(inst) \
	VIDEO_DEVICE_DEFINE(mt9m114_alif_vdev_##inst, DEVICE_DT_INST_GET(inst), NULL);
