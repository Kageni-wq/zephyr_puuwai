...
/* HAL ISP logging hook; applications may override the requested verbosity. */
int __weak log_level(void)
{
...
isp_bottom_done:
	if (!data->is_streaming) {
		alif_video_stream_stop(config->controller);
		data->curr_vid_buf = 0;
	}
		...
		struct video_format *fmt)
{
	...
	case ALIF_VIDEO_EP_IN:
		...
		ret = alif_video_set_format(config->controller, ALIF_VIDEO_EP_OUT, fmt);
		if (ret) {
		...
		struct video_format *fmt)
{
	...
	case ALIF_VIDEO_EP_IN:
		if (!port->port_fmt.pixelformat) {
			ret = alif_video_get_format(config->controller, ALIF_VIDEO_EP_OUT, fmt);
			if (ret) {
				return ret;
			}
...
static int isp_stream_start(const struct device *dev)
{
	...
	ret = alif_video_stream_start(config->controller);
	if (ret) {
...
static int isp_stream_stop(const struct device *dev)
{
	...
	ret = alif_video_stream_stop(config->controller);
	if (ret) {
		...
		struct video_caps *caps)
{
	...
	} else if (ep == ALIF_VIDEO_EP_IN) {
		if (config->controller) {
			/*
			 * Camera controlled output EP should have same fmt as
			 * ISP input EP.
			 */
			err = alif_video_get_caps(config->controller, ALIF_VIDEO_EP_OUT, caps);
			if (err) {
...
static int isp_flush(const struct device *dev, enum alif_video_endpoint ep, bool cancel)
{
	const struct isp_config *config = dev->config;
	struct isp_data *data = dev->data;
	...
	alif_video_flush(config->controller, ep, cancel);

	return 0;
}

static int isp_enqueue(const struct device *dev, enum alif_video_endpoint ep,
...
static int isp_dequeue(const struct device *dev, enum alif_video_endpoint ep,
...
static int isp_set_ctrl(const struct device *dev, unsigned int cid, void *value)
{
	...
	default:
		return alif_video_set_control(config->controller, cid, value);
	}
}
...
static int isp_get_ctrl(const struct device *dev, unsigned int cid, void *value)
{
	...
	default:
		return alif_video_get_control(config->controller, cid, value);
	}
}

#ifdef CONFIG_POLL
static int isp_set_signal(const struct device *dev, enum alif_video_endpoint ep,
...
int video_isp_init(const struct device *dev)
{
	...
	k_thread_name_set(k_work_queue_thread_get(&data->cb_workq), "isp_work_helper");
...
#define ALIF_VIDEO_REGISTER(inst) \
	VIDEO_DEVICE_DEFINE(isp_pico_vdev_##inst, DEVICE_DT_INST_GET(inst), DEVICE_DT_GET_OR_NULL(REMOTE_DEVICE(inst, 0)));
