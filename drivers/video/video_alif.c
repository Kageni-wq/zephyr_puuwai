...
static void alif_cam_work_helper(const struct device *dev)
{
	...
	if (config->axi_bus_ep) {
		...
		if (vbuf == NULL) {
			...
			data->curr_vid_buf = 0;
			data->is_streaming = false;
			alif_video_stream_stop(config->endpoint_dev);
...
static int alif_cam_set_fmt(const struct device *dev, enum alif_video_endpoint ep,
		       struct video_format *fmt)
{
	 ...
	 * or endpoint is neither ALIF_VIDEO_EP_OUT nor ALIF_VIDEO_EP_ALL.
	 */
	if (!bits_pp || !(ep == ALIF_VIDEO_EP_OUT || ep == ALIF_VIDEO_EP_ALL)) {
	...
	ret = alif_video_set_format(config->endpoint_dev, ep, fmt);
	if (ret) {
...
static int alif_cam_get_fmt(const struct device *dev, enum alif_video_endpoint ep,
		       struct video_format *fmt)
{
	const struct video_cam_config *config = dev->config;
	struct video_cam_data *data = dev->data;
	...
	if (!fmt || (ep != ALIF_VIDEO_EP_OUT && ep != ALIF_VIDEO_EP_ALL)) {
		return -EINVAL;
	}

	ret = alif_video_get_format(config->endpoint_dev, ep, fmt);
	if (ret) {
...
static int alif_cam_stream_start(const struct device *dev)
{
	...
	ret = alif_video_stream_start(config->endpoint_dev);
	if (ret) {
...
static int alif_cam_stream_stop(const struct device *dev)
{
	...
	ret = alif_video_stream_stop(config->endpoint_dev);
	if (ret) {
...
static int alif_cam_flush(const struct device *dev, enum alif_video_endpoint ep, bool cancel)
{
	struct video_cam_data *data = dev->data;
	uintptr_t regs = DEVICE_MMIO_GET(dev);
	struct video_buffer *vbuf = NULL;
...
static int alif_cam_enqueue(const struct device *dev, enum alif_video_endpoint ep,
...
static int alif_cam_dequeue(const struct device *dev, enum alif_video_endpoint ep,
...
static int alif_cam_set_ctrl(const struct device *dev, unsigned int cid, void *value)
{
	...
	ret = alif_video_set_control(config->endpoint_dev, cid, value);
	if (ret) {
		return -ENOTSUP;
	}
...
static int alif_cam_get_ctrl(const struct device *dev, unsigned int cid, void *value)
{
	...
	return alif_video_get_control(config->endpoint_dev, cid, value);
}

static int alif_cam_get_caps(const struct device *dev, enum alif_video_endpoint ep,
			struct video_caps *caps)
{
	const struct video_cam_config *config = dev->config;
	int err = -ENODEV;

	if (ep != ALIF_VIDEO_EP_OUT && ep != ALIF_VIDEO_EP_ALL) {
		LOG_ERR("Invalid end-point, ep = %d", ep);
		return -EINVAL;
	}

	err = alif_video_get_caps(config->endpoint_dev, ep, caps);
...
#ifdef CONFIG_POLL
static int alif_cam_set_signal(const struct device *dev, enum alif_video_endpoint ep,
...
static int alif_video_cam_init(const struct device *dev)
{
	...
	k_thread_name_set(k_work_queue_thread_get(&data->cb_workq), "alif_cam_work_helper");
...
#define ALIF_VIDEO_REGISTER(inst) \
	VIDEO_DEVICE_DEFINE(video_alif_vdev_##inst, DEVICE_DT_INST_GET(inst), DEVICE_DT_GET(REMOTE_DEVICE(inst, 0)));
