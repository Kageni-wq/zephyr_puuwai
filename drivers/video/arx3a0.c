...
struct arx3a0_data {
	struct video_ctrl gain_ctrl;
	struct video_ctrl exposure_ctrl;
...
static int arx3a0_set_fmt(const struct device *dev, enum alif_video_endpoint ep,
			  struct video_format *fmt)
{
	...
	drv_data->is_streaming = false;

	return 0;
}

static int arx3a0_get_fmt(const struct device *dev, enum alif_video_endpoint ep,
			  struct video_format *fmt)
{
	struct arx3a0_data *drv_data = dev->data;
...
static int arx3a0_stream_stop(const struct device *dev)
{
	...
	ret = arx3a0_write_reg(dev, ARX3A0_MODE_SELECT_REGISTER, 1, &val);
	if (ret) {
		LOG_ERR("Failed to stop streaming!");
		return ret;
	}
...
static int arx3a0_get_caps(const struct device *dev, enum alif_video_endpoint ep,
...
static int arx3a0_get_ctrl(const struct device *dev, unsigned int cid, void *value)
{
	...
	case VIDEO_CID_EXPOSURE:
		return arx3a0_read_reg(dev, ARX3A0_COARSE_INTEGRATION_TIME_REGISTER, 2, value);
	default:
		return -ENOTSUP;
	}
}

ALIF_VIDEO_API(arx3a0_driver_api,
...
{
	const struct arx3a0_config *config = dev->config;
	struct arx3a0_data *data = dev->data;
	uint16_t val;
	int ret;
	...
	ret = arx3a0_set_fmt(dev, ALIF_VIDEO_EP_OUT, &fmt);
	if (ret) {
		LOG_ERR("Unable to set default format. ret - %d", ret);
		return ret;
	}

	ret = video_init_ctrl(&data->gain_ctrl, dev, VIDEO_CID_GAIN,
		(struct video_ctrl_range){.min = 0, .max = 0x3fe000, .step = 1, .def = 0x10000});
	if (ret != 0) {
		return ret;
	}
	data->gain_ctrl.flags |= VIDEO_CTRL_FLAG_VOLATILE;
	ret = video_init_ctrl(&data->exposure_ctrl, dev, VIDEO_CID_EXPOSURE,
		(struct video_ctrl_range){.min = 0, .max = 0xffff, .step = 1, .def = 1});
	data->exposure_ctrl.flags |= VIDEO_CTRL_FLAG_VOLATILE;
	return ret;
}
...
#define ALIF_VIDEO_REGISTER(inst) \
	VIDEO_DEVICE_DEFINE(arx3a0_vdev_##inst, DEVICE_DT_INST_GET(inst), NULL);
