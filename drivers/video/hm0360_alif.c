...
#define DT_DRV_COMPAT himax_hm0360_alif
...
LOG_MODULE_REGISTER(hm0360_alif);
...
static int hm0360_hw_reset(const struct device *dev)
{
	...
	const struct gpio_dt_spec *gpios[] = {
		&cfg->resetn_gpio, &cfg->power_gpio, &cfg->xsleepn_gpio,
	};
	const gpio_flags_t flags[] = {
		GPIO_OUTPUT_ACTIVE, GPIO_OUTPUT_INACTIVE, GPIO_OUTPUT_ACTIVE,
	};
	int ret;

	for (size_t i = 0; i < ARRAY_SIZE(gpios); i++) {
		if (gpios[i]->port == NULL) {
			continue;
		}
		if (!gpio_is_ready_dt(gpios[i])) {
			return -ENODEV;
		}
		ret = gpio_pin_configure_dt(gpios[i], flags[i]);
		if (ret) {
			return ret;
		}
	}
	...
	if (cfg->resetn_gpio.port != NULL) {
		ret = gpio_pin_set_dt(&cfg->resetn_gpio, 0);
		if (ret) {
			return ret;
		}
	}
	k_usleep(400);
	if (cfg->xsleepn_gpio.port != NULL) {
		ret = gpio_pin_set_dt(&cfg->xsleepn_gpio, 0);
		if (ret) {
			return ret;
		}
	}
...
static int hm0360_set_fmt(const struct device *dev, enum alif_video_endpoint ep,
						 struct video_format *fmt)
{
...
static int hm0360_get_fmt(const struct device *dev, enum alif_video_endpoint ep,
		struct video_format *fmt)
{
	struct hm0360_data *drv_data = dev->data;
...
static int hm0360_stream_set(const struct device *dev, bool enable)
{
	if (enable) {
		return hm0360_stream_start(dev);
	} else {
		return hm0360_stream_stop(dev);
	}
}

static int hm0360_get_caps(const struct device *dev, enum alif_video_endpoint ep,
...
static int hm0360_init(const struct device *dev)
{
	...
	ret = hm0360_set_fmt(dev, ALIF_VIDEO_EP_OUT, &fmt);
	if (ret) {
		return ret;
	}
	 ...
	 */
	return i2c_configure(cfg->i2c.bus, I2C_MODE_CONTROLLER | I2C_SPEED_SET(I2C_SPEED_FAST));
}

/* Device configuration */
#define HM0360_DEFINE(inst)                                                                  \
	static const struct hm0360_config hm0360_config_##inst = {                            \
		.i2c = I2C_DT_SPEC_INST_GET(inst),                                            \
		.resetn_gpio = GPIO_DT_SPEC_INST_GET_OR(inst, reset_gpios, {0}),               \
		.power_gpio = GPIO_DT_SPEC_INST_GET_OR(inst, power_gpios, {0}),                \
		.xsleepn_gpio = GPIO_DT_SPEC_INST_GET_OR(inst, xsleep_gpios, {0}),             \
		.fps = DT_INST_PROP_OR(inst, fps, 60),                                        \
	};                                                                                  \
	static struct hm0360_data hm0360_data_##inst = {                                      \
		.capture_mode = CONTINUOUS_CAPTURE,                                          \
		.numframes = 1,                                                             \
	};                                                                                  \
	DEVICE_DT_INST_DEFINE(inst, hm0360_init, NULL, &hm0360_data_##inst,                    \
			      &hm0360_config_##inst, POST_KERNEL, CONFIG_VIDEO_INIT_PRIORITY,    \
			      &hm0360_driver_api);                                               \
	VIDEO_DEVICE_DEFINE(hm0360_alif_vdev_##inst, DEVICE_DT_INST_GET(inst), NULL);

DT_INST_FOREACH_STATUS_OKAY(HM0360_DEFINE)
