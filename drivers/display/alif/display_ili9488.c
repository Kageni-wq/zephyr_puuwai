...
int ili9488_regs_init(const struct device *dev)
{
	...
	r = alif_ili9xxx_transmit(dev, ILI9488_FRMCTR1, regs->frmctr1,
			     ILI9488_FRMCTR1_LEN);
	if (r < 0) {
		return r;
	}
	...
	r = alif_ili9xxx_transmit(dev, ILI9488_DISCTRL, regs->disctrl,
			     ILI9488_DISCTRL_LEN);
	if (r < 0) {
		return r;
	}
	...
	r = alif_ili9xxx_transmit(dev, ILI9488_PWCTRL1, regs->pwctrl1,
			     ILI9488_PWCTRL1_LEN);
	if (r < 0) {
		return r;
	}
	...
	r = alif_ili9xxx_transmit(dev, ILI9488_PWCTRL2, regs->pwctrl2,
			     ILI9488_PWCTRL2_LEN);
	if (r < 0) {
		return r;
	}
	...
	r = alif_ili9xxx_transmit(dev, ILI9488_VMCTRL, regs->vmctrl,
			     ILI9488_VMCTRL_LEN);
	if (r < 0) {
		return r;
	}
	...
	r = alif_ili9xxx_transmit(dev, ILI9488_PGAMCTRL, regs->pgamctrl,
	...
	r = alif_ili9xxx_transmit(dev, ILI9488_NGAMCTRL, regs->ngamctrl,
