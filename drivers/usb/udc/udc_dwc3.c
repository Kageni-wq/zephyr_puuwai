...
static void udc_dwc3_ep0_status_done(udc_dwc3_driver_t *drv)
{
	...
	drv->actual_length = 0;
	drv->num_bytes = 0;
	if (drv->ep0_expect_in) {
		drv->udc_dwc3_data_out_cb(drv, USB_CONTROL_EP_OUT);
	} else {
		drv->udc_dwc3_data_in_cb(drv, USB_CONTROL_EP_IN);
	}
	drv->ep0_state = EP0_SETUP_PHASE;
	...
	if (endp_number == USB_CTRL_PHY_EP0 || endp_number == USB_CTRL_PHY_EP1) {
		...
		if (event_type == USB_DEPEVT_XFERNOTREADY) {
			if (event_status == USB_DEPEVT_STATUS_CONTROL_DATA) {
				...
				priv->status_ready = true;
				if (priv->status_requested) {
			...
			case EP0_SETUP_PHASE:
				DWC3DATA(drv)->status_ready = false;
				DWC3DATA(drv)->status_requested = false;
...
static int udc_dwc3_tx(const struct device *dev, uint8_t ep, struct net_buf *buf)
{
	...
	if (ret == 0) {
		udc_ep_set_busy(udc_get_ep_cfg(dev, ep), true);
	}
		...
		struct net_buf *buf)
{
	...
	if (EP_NUM(epcfg->addr) == 0U && udc_get_buf_info(buf)->status) {
		priv->status_requested = true;
		udc_ep_set_busy(epcfg, true);
		ret = 0;
		if (priv->status_ready) {
			ret = udc_dwc3_ep0_send_recv_status(&priv->drv,
						     USB_EP_DIR_IS_IN(epcfg->addr));
		}
		irq_unlock(lock_key);
		return ret;
	}
...
static int udc_dwc3_ep_dequeue(const struct device *dev, struct udc_ep_config *epcfg)
{
	...
	ret = udc_dwc3_stop_transfer(&priv->drv, EP_NUM(epcfg->addr),
				     USB_EP_DIR_IS_IN(epcfg->addr), USB_DEPCMD_FORCERM);
	if (ret != 0) {
		return ret;
	}
	udc_ep_set_busy(epcfg, false);
	udc_ep_cancel_queued(dev, epcfg);
	return 0;
}
...
static void handle_data_out(struct udc_dwc3_data *priv, uint8_t epnum, uint16_t received)
{
	const struct device *dev = priv->dev;
	...
	buf = udc_buf_get(cfg);
	if (buf != NULL) {
		if (received > net_buf_tailroom(buf)) {
