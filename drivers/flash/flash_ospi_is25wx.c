				...
				size_t length)
{
	...
	const struct alif_flash_ospi_config *f_param = dev_cfg;
	struct alif_flash_ospi_dev_data *dev_data = dev->data;
	 ...
	 */
	size_t addr_offset = address & (f_param->flash_param.write_block_size - 1);

	if (addr_offset != 0 && length > 0) {
		uint8_t temp_buf[4] = {0};
		size_t prefix = f_param->flash_param.write_block_size - addr_offset;

		if (prefix > length) {
			prefix = length;
		}

		uint32_t aligned_addr = address & ~(f_param->flash_param.write_block_size - 1);
	...
	size_t aligned_len = length & ~(f_param->flash_param.write_block_size - 1);
	...
	size_t blocks = aligned_len / f_param->flash_param.write_block_size;
	...
	while (blocks) {
		...
		address += (data_cnt * f_param->flash_param.write_block_size);
		data_ptr += (data_cnt * f_param->flash_param.write_block_size);
	}
				 ...
				 size_t length)
{
	...
	if (addr_offset != 0 && length > 0) {
		...
		memset(temp_buf, f_param->flash_param.erase_value, sizeof(temp_buf));
	...
	cnt = aligned_len / f_param->flash_param.write_block_size;
	...
	while (ret == 0 && cnt) {
		...
		data_cnt = (OSPI_MAX_TX_COUNT - ((address / f_param->flash_param.write_block_size) %
							OSPI_MAX_TX_COUNT));
		if (data_cnt > cnt) {
			data_cnt = cnt;
		}
...
static int flash_is25wx_ospi_erase(const struct device *dev, off_t addr, size_t len)
{
	...
	const struct alif_flash_ospi_config *f_param = dev_config;
	...
	while (len) {
		...
		if (len == (dev_config->sector_size *
				dev_config->num_of_sector)) {
			ret = erase_chip(dev);
			if (ret != 0) {
				break;
			}
			len -= dev_config->sector_size *
			       dev_config->num_of_sector;
		} else {
