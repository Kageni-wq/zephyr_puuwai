				...
				size_t length)
{
	...
	const struct mx_flash_ospi_config *f_param = dev_cfg;
	...
	if (length % f_param->flash_param.write_block_size
		|| !(IS_ALIGNED(buffer, f_param->flash_param.write_block_size))) {
		return -EINVAL;
	}
	...
	length /= f_param->flash_param.write_block_size;
				 ...
				 size_t length)
{
	...
	cnt = length / f_param->flash_param.write_block_size;
	data_ptr = (uint8_t *)buffer;
...
static int flash_mx66uw_erase(const struct device *dev, off_t addr, size_t len)
{
	...
	const struct mx_flash_ospi_config *f_param = dev_config;
