...
#define DEV_DATA(dev) ((struct adc_alif_data *)((dev)->data))
...
{
	struct adc_alif_data *data = CONTAINER_OF(ctx, struct adc_alif_data, ctx);
			     ...
			     uint8_t active_channels, uint8_t conv_mode)
{
	...
	if (conv_mode == ADC_CONV_MODE_SINGLE_SHOT) {
		/* Single-shot: single sample per conversion */
		needed_buffer_size = sizeof(uint32_t);
	} else {
		/* Continuous/multi-channel: buffer size based on number of active channels */
		needed_buffer_size = (size_t)active_channels * sizeof(uint32_t);
	}
...
#define ADC_ALIF_INIT(inst)                                                                        \
	...
	static struct adc_alif_data data_##inst = {                                                            \
