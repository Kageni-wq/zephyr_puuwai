/* Alif H:4 transport with ES0 radio power control. */
...
/** Setup the HCI transport, which usually means to reset the Bluetooth IC
  ...
  */
static int alif_hci_transport_setup(const struct device *uart)
{
	h4_discard(uart, 32);
	return 0;
}
...
static int h4_open(const struct device *dev)
{
	...
	ret = alif_hci_transport_setup(cfg->uart);
	if (ret < 0) {
		(void)stop_using_es0();
		return -EIO;
	}
...
static int alif_hci_transport_teardown(const struct device *dev)
{
	return 0;
}
...
static int h4_close(const struct device *dev)
{
	...
	err = alif_hci_transport_teardown(cfg->uart);
	if (err < 0) {
		return err;
	}
