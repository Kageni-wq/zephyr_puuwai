...
/**
 * @brief Send data to the device (supervisor threads only).
 *
 ...
 */
static inline int ipm_poll_out(const struct device *ipmdev, uint32_t id,
				const void *data, int size, k_timeout_t timeout)
{
...
/**
 * @brief Receive data from the device (supervisor threads only).
 *
 ...
 * @retval -EAGAIN If timeout.
 */
static inline int ipm_poll_in(const struct device *ipmdev, uint32_t id,
				const void *data, int size, k_timeout_t timeout)
{
	const struct ipm_driver_api *api =
		(const struct ipm_driver_api *)ipmdev->api;
