...
class AlifImageBinaryRunner(ZephyrBinaryRunner):
    ...
    def __init__(self, cfg, device,
        ...
        self.exe_dir = os.getenv("ALIF_SE_TOOLS_DIR")
    ...
    def flash(self, **kwargs):
        ...
        for tool, args in (
            (self.gen_toc, ['-f', self.exe_dir + self.cfg_op_file]),
            (self.write_toc, ['-p']),
        ):
            command = [str(Path(self.exe_dir) / tool)]
            if tool.endswith('.py'):
                command.insert(0, sys.executable)
            self.check_call(command + args, cwd=self.exe_dir, **kwargs)
    ...
    def do_run(self, command, **kwargs):
        ...
        if command in ('attach', 'debugserver'):
        ...
        if not self.verify_tool_config(self.logger, self.device):
            raise RuntimeError("Alif SE tool configuration does not match the target")

        # Update SE communication port.
        if not self.verify_se_port(self.logger, self.com_port):
            raise RuntimeError("Failed to update the Alif SE communication port")
    ...
    def get_itcm_address(self, logger):
        """Retrieve itcm address from DTS."""
        try:
            node = self.build_conf.edt.label2node['itcm']
            addr = node.props['global_base'].val
        except (OSError, KeyError, TypeError, ValueError) as err:
            logger.error("Parsing itcm address failed: %s", err)
            raise RuntimeError("Cannot determine ITCM load address") from err
        return addr

    def prepare_json(self, logger, build_core, fls_addr, fls_size):
        """Prepare JSON for ToC generation."""
        cfg_ip_path = os.path.join(self.exe_dir, self.cfg_ip_file.lstrip('/'))
        cfg_op_path = os.path.join(self.exe_dir, self.cfg_op_file.lstrip('/'))
        ...
        try:
            ...
            raise RuntimeError("Cannot prepare Alif image configuration")
        ...
        if fls_addr == 0:
            itcm_addr = self.get_itcm_address(logger)
            if itcm_addr == 0:
        ...
        elif fls_addr >= self.mram_base_addr and fls_addr <= self.mram_base_addr + (fls_size * 1024):
